...
class Plugin {
    ...
    const DAAN_BETTER_CHECKOUT_REWRITE_TEXT_FIELDS = [
            ...
            'Validate'                                                                                                                                                                                                                => 'Check',
            'City'                                                                                                                                                                                                                    => 'Billing City',
            'Zip/Postal code'                                                                                                                                                                                                         => 'Billing Postal / ZIP Code',
            'Country'                                                                                                                                                                                                                 => 'Billing Country',
            'State/Province'                                                                                                                                                                                                          => 'Billing State / Province',
    ];

    const DAAN_BETTER_CHECKOUT_EMPTY_TEXT_FIELDS = [
            'Payment Info',
            'Excluding %1$s&#37; tax',
    ...
    private function init() {
        ...
        /**
         * Add "Your Details" to the top of the form.
         */
        add_filter( 'render_block', [ $this, 'add_your_details_header' ], 10, 2 );
    ...
    public function modify_text_fields( $translation, $text, $domain ) {
        ...
        if ( in_array( $text, self::DAAN_BETTER_CHECKOUT_EMPTY_TEXT_FIELDS ) ) {
            return '';
        }
    ...
    /**
     * Modify the Latest Posts block to use our own callback. @param $settings
     *
     * @param $name
     *
     * @return mixed
     * @see LatestPosts::render()
     *
     */
    public function add_your_details_header( $block_content, $block ) {
        if ( $block['blockName'] === 'edd/checkout' ) {
            $block_content = preg_replace(
                    '/(<form[^>]*id=["\']edd_purchase_form["\'][^>]*>)/i',
                    '$1<div id="edd_purchase_form_title"><h2>' . __( 'Your details', 'edd-better-checkout' ) . '</h2></div>',
                    $block_content,
                    1
            );
        }
